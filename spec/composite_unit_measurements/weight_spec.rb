...
RSpec.describe CompositeUnitMeasurements::Weight do
  subject(:weight) { described_class }
  ...
  describe "#parse" do
    ...
    context "when invalid string is passed" do
      it "raises an error" do
        expect { subject.parse("8 lb 12 ozz") }.to raise_error(UnitMeasurements::ParseError)
        expect { subject.parse("8 lbb 12 ozz") }.to raise_error(UnitMeasurements::ParseError)
        expect { subject.parse("8 ## 12 ounces") }.to raise_error(UnitMeasurements::ParseError)
        expect { subject.parse("8 lbms 12 oz") }.to raise_error(UnitMeasurements::ParseError)
        expect { subject.parse("8 lbms 12 oz") }.to raise_error(UnitMeasurements::ParseError)
        expect { subject.parse("8 poundss 12 ouncess") }.to raise_error(UnitMeasurements::ParseError)
        expect { subject.parse("8 lbs 12 ounc") }.to raise_error(UnitMeasurements::ParseError)

        expect { subject.parse("2 sts 6 lbs") }.to raise_error(UnitMeasurements::ParseError)
        expect { subject.parse("2 stoness 6 poundss") }.to raise_error(UnitMeasurements::ParseError)
